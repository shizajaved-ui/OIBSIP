    ...
    <div className={`min-h-[calc(100vh-72px)] md:min-h-[calc(100vh-104px)] ${showDoodle ? 'doodle-bg' : 'bg-char-900'} flex justify-center ${isFloating ? (fullMobile ? 'items-start py-0 md:py-20 px-0 md:px-6' : 'items-start py-10 md:py-20 px-4 md:px-6') : 'items-stretch'}`}>
      ...
      */}
      ...
      >
        ...
        >
          {(title || subtitle) && (
            <div className="w-full bg-basil md:bg-basil/[0.08] rounded-[32px] md:rounded-[48px] border-2 border-basil/10 p-8 md:p-12 text-center mb-10 md:mb-14 shadow-lg relative overflow-hidden group">
              ...
              {title && (
                <h1 className="relative z-10 font-display text-4xl md:text-5xl font-black tracking-tight text-white md:text-char-950 mb-3 md:mb-4 transition-transform group-hover:scale-[1.01] drop-shadow-md">
                  {title}
                </h1>
              )}
              {subtitle && (
                <p className="relative z-10 text-base md:text-lg font-bold text-white/90 md:text-basil/70 italic tracking-wide max-w-2xl mx-auto">
                  {subtitle}
                </p>
              )}
