...
const PageLayout = ({ children, title, subtitle, width = '3xl', showDoodle = true, isFloating = false, useDoodleOverlay = false }) => {
  ...
  return (
    <div className={`min-h-[calc(100vh-80px)] ${showDoodle ? 'doodle-bg' : 'bg-char-900'} flex justify-center ${isFloating ? 'items-start py-10 md:py-20 px-4 md:px-6' : 'items-stretch'}`}>
      {/*
      ...
      */}
      <div
        className={`w-full ${widthClasses[width] || 'max-w-3xl'} ${useDoodleOverlay ? 'relative overflow-hidden' : 'bg-char-800'} shadow-2xl flex flex-col items-center py-10 md:py-16 px-4 md:px-12 transition-all duration-500 ${isFloating ? 'rounded-[40px] md:rounded-[64px] border border-char-950/5' : ''}`}
